class Gridded:
    ...
    def GridData(self, tlim, vec_lat, vec_lon, SD=[], SD_unc=[], SIT=[], SIT_unc=[], t=[], Frb=[], Frb_unc=[]):
        #import pdb;
        import numpy as np
                ...
                if self.sd_list[i][j]: 
                    ...
                    for n in range(len(self.td_list[i][j])):
                        if (self.td_list[i][j][n] - t0).days < tlim:
                            ...
                            count +=1
                            flag=0
                            print("%6i %6i %6i %6i %6.2f %6.2f %6.2f %6.2f %12.6f %12.0f" % (i,j,tn,(self.td_list[i][j][n] - t0).days,self.sd_list[i][j][n],self.sit_list[i][j][n],self.Frb_list[i][j][n],self.lat_list[i][j][n],self.lon_list[i][j][n],(self.td_list[i][j][n] - t0).total_seconds()),output)
                        else:
                            count += 1
                            flag = 1
                            avgSD = np.nanmean(sd)
                            ...
                            if np.isnan(avgFrb):
                                ...
                                avgDT = np.mean(dt0[np.isnan(Frb)==0])
                                ...
                                print((i,j,tn,avgSD,stdSD,lnSD,avgFrb,stdFrb,lnFrb,avgSIT,stdSIT,lnSIT,avgLat,avgLon,avgDT),file=output)
                            ...
                            sd = np.append(sd,self.sd_list[i][j][n])
                            sd_unc = np.append(sd_unc,self.sd_unc_list[i][j][n])
                            Frb = np.append(sd,self.Frb_list[i][j][n])
                            frb_unc = np.append(frb_unc,self.Frb_unc_list[i][j][n])
                    ...
                    for n in range(len(self.td_list[i][j])):
                        if (self.td_list[i][j][n] - t0).days < tlim:
                            ...
                            print("%6i %6i %6i %6.2f %6.2f %6.2f %6.2f %12.6f %12.6f %12.0f" % (i,j,tn,(self.td_list[i][j][n] - t0).days,self.sd_list[i][j][n],self.sit_list[i][j][n],self.Frb_list[i][j][n],self.lat_list[i][j][n],self.lon_list[i][j][n],(self.td_list[i][j][n] - t0).total_seconds()),output) 
                            if count == len(self.td_list[i][j]): #done to include the last element which is otherwise excluded??
                                ...
                                if np.isnan(avgFrb):
                                    ...
                                    avgDT = np.mean(dt0[np.isnan(Frb)==0])
                                    avgTime = t0+timedelta(seconds=avgDT)                 
                                    # print i,j,t0,t0+timedelta(seconds=avgDT),avgTime
                                    ...
                                    print(i,j,tn,avgSD,stdSD,lnSD,avgFrb,stdFrb,lnFrb,avgSIT,stdSIT,lnSIT,avgLat,avgLon,avgDT,output)
                    ...
                    if flag == 0:
                        ...
                        if np.isnan(avgFrb):
                            ...
                            avgLat = np.mean(lat[np.isnan(lat)==0])
                            avgLon = np.mean(lon[np.isnan(lon)==0])
                            avgDT = np.nanmean(dt0[np.isnan(Frb)==0])
                            avgTime = t0+timedelta(seconds=avgDT)                 
                            # print i,j,tn,avgSD,flag,avgLat,avgLon,avgDT
                            # avgTime = t0+timedelta(seconds=avgDT)                 
                            # print i,j,t0,t0+timedelta(seconds=avgDT),avgTime
                            ...
                            print(i,j,tn,avgSD,stdSD,lnSD,avgFrb,stdFrb,lnFrb,avgSIT,avgLat,avgLon,avgDT,output)
