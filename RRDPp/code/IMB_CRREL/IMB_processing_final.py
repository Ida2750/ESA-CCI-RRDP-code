...
def write_info_table(self, files, MassDate, TempDate, MetDate, SD, SIT):
    """
    Parameters
    ----------
    ID
    ...
    ofile = 'IMB_info_table.dat'
    output = open(ofile, 'a')
    
    print('{:^s} & {:^s} & {:^s} & {:^s} & {:^6.2f} & {:^6.2f} & {:^6.2f} & {:^s} & {:^s} & {:^6.2f} & {:^6.2f} \\'.format(
        self.obsID, position, date_start, date_end, intervals[0], intervals[1], intervals[2], s_SD, s_SIT, np.nanmean(self.pp_flag), np.nanmean(self.unc_flag)), file=output)
    output.close()
...
for dirr in os.listdir(dataDir):
    ...
    if dirr.startswith('Snow'):
        ...
        for dirrr in os.listdir(path):
            ...
            SD = Mass_data['Snow_Depth'].astype(float)   # meters
            ...
            write_info_table(dataOut, files, Mass_data['Date'],Temp_data['Date'],Met_data['Date'], SD, SIT)
            
            print(sum(lnSD), len(SD[~np.isnan(SD)]))
